...
use App\Entity\WorkspaceSharedFile;
...
class WorkspaceSharedFilesController extends Controller
{
    public function indexAction()
    {
        ...
        $workspaceFiles = $this->getDoctrine()->getRepository('App:WorkspaceSharedFile')
            ->search($account, $offset, $limit, $filters, $sort, $getDeleted, false);

        $workspaceFileCount = $this->getDoctrine()->getRepository('App:WorkspaceSharedFile')
    ...
    public function uploadAction(S3ClientFactory $s3ClientFactory)
    {
        ...
        for ($i = 0; $i < $filesCount; $i++) {
            ...
            $prefix = $this->getParameter('aws_workspace_shared_files_folder');
            ...
            $fileData = new WorkspaceSharedFile();
    ...
    public function deleteAction(Request $request, WorkspaceSharedFile $workspaceSharedFile)
    {
        if ($this->checkToken() === false) {
            return $this->getResponse()->error(ExceptionMessage::INVALID_TOKEN, 401);
        }
        ...
        if ($account != $workspaceSharedFile->getAccount()) {
            return $this->getResponse()->error(ExceptionMessage::NO_ACCESS);
        }
        ...
        $workspaceSharedFile->setDeletedAt(new DateTime());

        $em = $this->getDoctrine()->getManager();
        $em->persist($workspaceSharedFile);
        $em->flush();

        return $this->getResponse()->success(['removed_file' => $workspaceSharedFile->getId()]);
    }


    public function publishAction(Request $request, WorkspaceSharedFile $workspaceSharedFile)
    {
        if ($this->checkToken() === false) {
            return $this->getResponse()->error(ExceptionMessage::INVALID_TOKEN, 401);
        }
        ...
        $workspaceSharedFile->setDeletedAt(null);

        $em = $this->getDoctrine()->getManager();
        $em->persist($workspaceSharedFile);
        $em->flush();

        return $this->getResponse()->success(['published_file' => $workspaceSharedFile->getId()]);
    }
    ...
    public function downloadAction(
        ...
        Request $request,
        WorkspaceSharedFile $workspaceSharedFile
    )
    {
        ...
        $fileName = $workspaceSharedFile->getServerFilename();
        ...
        try {
            ...
            return new Response($result['Body'], 200, [
                ...
                'Content-Disposition' => sprintf(
                    ...
                    $workspaceSharedFile->getOriginalFilename()
                ),
            ]);
        } catch (S3Exception $e) {
    ...
    public function updateDescriptionAction(WorkspaceSharedFile $workspaceSharedFile)
    {
        if ($this->checkToken() === false) {
            return $this->getResponse()->error(ExceptionMessage::INVALID_TOKEN, 401);
        }
        ...
        $workspaceSharedFile->setDescription($newDescription);

        $em->persist($workspaceSharedFile);
        $em->flush();

        return $this->getResponse()->success(['updated_description_file' => $workspaceSharedFile->getId()]);
    }
}
